...
export const registerMemberSC = async (req, res, next) => {
    ...
    try {
        ...
        const existingUser = await firestore.collection('SC Members').where('NIM', '==', NIM_database).get();
        ...
        await firestore.collection('SC Members').add(member.toJson());
...
export const deleteMemberSC = async (req, res, next) => {
    const { name, NIM } = req.body;
    ...
    try {
        ...
        if (memberID === memberIDValidate) {
            await firestore.collection('SC Members').doc(memberID).delete();
...
export const verifySCUser = async (req, res, next) => {
...
async function getUserIdFromNIM(NIM){
    ...
    const userQuery = await firestore.collection('SC Members').where('NIM', '==', NIM).get();
...
async function getUserIdFromName(name){
    ...
    const userQuery = await firestore.collection('SC Members').where('name', '==', name).get();
